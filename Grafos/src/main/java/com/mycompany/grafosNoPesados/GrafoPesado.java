...
public class GrafoPesado {
		...
		public boolean existeAdyacencia(int posicionOrigen, int posicionDestino) {
				...
				for (int i = 0; i < listaDeAycentes.size(); i++) {
						AdyacentesConPeso ad = listaDeAycentes.get(i);
						if (ad.getIndiceVertice() == posicionDestino) {
								return true;
						}
				}
		...
		public Iterable<Integer> adyacentesDelVertice(int posicionDelVertice) {
				...
				for (int i = 0; i < listaDeAdyacentes.size(); i++) {
						AdyacentesConPeso ad = listaDeAdyacentes.get(i);
						soloVertices.add(ad.getIndiceVertice());

				}

				return soloVertices;                /// CODIGO OPCIONAL
